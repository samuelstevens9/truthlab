...
function aa_scripts() {
	if ( $GLOBALS[ 'pagenow' ] != 'wp-login.php' && ! is_admin() ) {
		...
		wp_enqueue_script( 'jquery-blockui', get_stylesheet_directory_uri() . '/assets/js/vendors/jquery.blockUI.js',
			array( 'jquery' ) );
		wp_enqueue_script( 'aos', get_stylesheet_directory_uri() . '/assets/js/vendors/aos.js', [] );
		wp_enqueue_script( 'scrollmagic',
			get_stylesheet_directory_uri() . '/assets/libs/scrollmagic/ScrollMagic.min.js', [] );
		wp_enqueue_script( 'tweenmax', 'https://cdnjs.cloudflare.com/ajax/libs/gsap/1.20.3/TweenMax.min.js', [] );
		wp_enqueue_script( 'tweenfx',
			get_stylesheet_directory_uri() . '/assets/libs/scrollmagic/plugins/animation.gsap.min.js',
			[] );
		/*wp_enqueue_script( 'slickjs', get_stylesheet_directory_uri() . '/assets/libs/slick/slick.min.js',
			array( 'jquery' ) );*/

		wp_register_script( 'aa_vendorsJs',
			get_template_directory_uri() . '/assets/js/vendors.min.js' ); // Custom scripts
		wp_enqueue_script( 'aa_vendorsJs' ); // Enqueue it!

		wp_register_script( 'aa_customJs',
			get_template_directory_uri() . '/assets/js/custom.min.js' ); // Custom scripts
...
function aa_styles() {
	...
	wp_enqueue_style( 'aos', get_stylesheet_directory_uri() . '/assets/css/vendors/aos.css' );
...
function tl_free_whitepaper_form_func( $atts ) {
	...
	ob_start();
	?>
    <form class="whitepaper_form" target="_blank" action="<?php echo $a[ 'target' ]; ?>" id="gated_content"
          method="POST">
        <small>
            <div id="form-notes" style="color:red;"></div>
        </small>
        ...
        <input type="hidden" name="tl_free_whitepaper_form" value="<?php echo $a[ 'redirect' ]; ?>"
               id="tl_free_whitepaper_form">
        <div class="blog-subscribe-checkbox"><input name="blog" type="checkbox" checked><span style="margin-left:5px">Sign me up for the Truthlab Blog Subscription</span>
        </div>
